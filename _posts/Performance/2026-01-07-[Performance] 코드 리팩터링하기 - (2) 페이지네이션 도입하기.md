...
기존은 2분이 넘어 타임아웃이 발생했었는데, 이번에 Max값은 24초로, 약 96초(80%) 감소하며 성능 개선이 되었음을 알 수 있다.

그러나 여전히 절반 이상이 1200ms 이상이며, 사용자 수가 지속적으로 늘어나면 다시 같은 문제가 발생할 것을 염두에 두어야 한다.
...
따라서 이에 대해 추가적으로 성능 개선을 진행하고자 한다.
