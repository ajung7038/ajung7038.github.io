...
- `Max`: 1,220ms -> 374ms **<U>(69.3% 감소)</U>**
- `Connect WS(p99)`: 229ms -> 243ms **<U>(6.1% 증가)</U>**
- `Check result(Max)`: 366ms -> 293ms **<U>(19.9% 감소)</U>**
- `Close(Max)`: 207ms -> 249ms **<U>(20.3% 증가)</U>**
- `표준편차`: 50ms -> 39ms **<U>(22% 감소)</U>**

미미하지만 전체 성능이 소폭 개선되었음을 알 수 있다.
...
또한, 표준편차가 22% 감소하였다는 사실을 알 수 있는데, 이는 시스템의 안정성이 조금이나마 확보된 결과로 볼 수 있을 것이다!

연결 종료 시에는 특이하게도 증가하였는데, p95부터 증가하는 모습을 보인다. 또한, 나머지는 증가하지만, min 부분은 기존보다 11ms -> 9ms로 줄어들었음을 알 수 있다. 따라서 Multi-Puller 도입에 의해 무언가 변화가 생겼음을 의심할 수 있을 것이다.

또한, Connect WS의 안정성을 챙긴 대신 전체적인 지연이 소폭 발생하였음을 알 수 있다. 득과 실을 생각해보았을 떄, Max Latency를 약 70% 줄일 수 있었기 때문에 WS 연결 과정에서 발생한 14ms 정도의 지연은 테스트상 오차라고 판단하였다.
<br/>
(실제로 해당 부분을 확인하기 위해 테스트를 세 번 더 거친 결과, Connect WS의 부분은 빠를 때도, 느릴 때도 있었다. 오차 +-20ms로 수렴한 것 같다.)
