...
- 무엇보다 테스트 결과, API 요청과 결과 사이의 p99값이 0.1이므로 해당 가설도 거짓이다.
...
### ✨ 가설 11: 과도한 Context Switching에 의해 Poller 스레드가 제때 CPU를 받지 못해 지연이 발생한다. (참)
...
결과적으로 동일한 환경에서 테스트를 두 번 진행한 결과, 성공률은 모두 100%를 달성할 수 있었으며, 튀는 현상이 1000ms -> 300ms로 감소했음을 알 수 있다.
...
연결 종료 시에도 종료 요청 패킷, 소켓 자원 반납 등 Poller을 거치기 때문에 PollerThreadCount의 값을 증가시킴으로써 응답 속도 향상이 되었던 것 같다.

## 🫧 보완점

그러나 여전히 고칠 점은 많다. 마지막 테스트 결과에서도 응답이 고르지 않게 분포하였으며, 과부하 상황에서 서버가 안정적으로 처리하지 못하고 있는 것 같았다.
...
특히, GCP 모니터링으로 관찰 결과 높아봤자 47.2%였기 때문에 CPU 부족으로 인한 불안정한 처리보다는 처리 흐름 상 어떠한 문제가 있을 것으로 예측할 수 있었다.
