---
title: "[Performance] Multi-Poller 적용을 통한 웹소켓 병목 제거 및 Max Latency 81.7% 감소"
categories:
  - Performance
tags:
toc: true
...
또한, 사용자가 WS 연결 후 3분의 입력 텀을 두어 사용자의 패턴을 구현하고자 했다.
...
게다가 약 1,000ms 이상의 튀는 현상이 지속되어 발생하는 것이다.
...
- 모든 튀는 지점의 응답 시간이 1,000ms 이상을 웃돌고 있다.
...
### ✨ 가설 5: 스레드가 부족하여 큐에 요청이 쌓이고 있을 것이다. (거짓)
- waiting thread는 16이 넘어가는 반면, 지속해서 Runnable 스레드는 7개를 웃돌고 있었다.
- Tomcat의 기본 스레드 풀 개수는 200개이며, 커스텀하지 않았다.
- 그러나 waiting thread는 16이 넘어가는 반면, 지속해서 Runnable 스레드는 7개를 웃돌고 있었다.
- 게다가 스트레스 테스트 결과 timed-waiting 스레드 수는 210개인 반면, Runnable 스레드는 7개를 유지하는 것을 확인하였다.

![alt text](../../assets/image/Performance/sleepTest/hypo5-2.png)

- 스레드가 추가로 늘어나지 않는 것으로 보아, 스레드 부족의 문제는 아님을 알 수 있다.

<br/>


### ✨ 가설 6: Poller가 Block 되어 있어 처리 시 지연이 발생할 것이다. (거짓)

이에 대해 들어가기 전, 간단하게 Java NIO에 관련하여 언급하도록 하자.
...
- 여기서 `Poller`는 Tomcat이 NIO를 사용하기 위해 만든 내부 객체이다.

<br/>

Poller 스레드 run() 메서드에는 다음과 같은 코드가 포함되어 있다.
<br/>
(정확히 말하면 Selector 클래스 내 메서드를 포함한다.)


- select() : 등록된 채널이 깨울 때까지 block 상태이다.
- select(timeout) : select()와 마찬가지로 Blocking 방식으로 동작하지만, 지정된(timeout) 시간까지만 Block된다. 해당 시간 안에 이벤트가 발생하면 그 즉시 반환하고, 아무 이벤트가 발생하지 않으면 timeout 후 0을 반환한다.
- selectNow() : Non-Blocking으로 동작하며, select와 달리 준비된 채널이 없어도 Block되지 않는다.

![alt text](../../assets/image/Performance/sleepTest/javaNio.png)

해당 코드는 wakeupCounter > 0이면 selectNow()를 호출하고, 그렇지 않으면 select(timeout)을 호출하고 있음을 알 수 있다.

이때 wakeUp()은 Poller에 이벤트가 추가되거나, 이미 등록된 이벤트의 상태 (읽기 가능, 쓰기 가능 등)가 바뀔 때 일어나게 된다.

또한, 내부적으로 코드를 확인했을 때 이때의 NioEndPoint.this.selectorTimeout의 값은 정확히 1000ms(1초)임을 알 수 있다.

<br/>

#### 💡 가설 검증

위의 내용을 기반으로, select(1000)을 호출한 이후 원래대로라면 select()의 리턴값을 받고 깨어나야 하지만, 다량의 스레드 처리 및 병목에 의해 해당 파트가 제대로 실현되지 않고 있음으로 판단하였다.
...
요청 처리 과정을 대략적으로 정리하자면 다음과 같다.

1. 요청 들어옴
2. TCP 3-way Handshake
3. Acceptor가 accept()를 통해 커넥션 객체 생성
    - fd를 받아 SocketChannel 객체 생성
    - PollerEvent로 감싸 Poller에 등록
4. 이후 HTTP Get Upgrade 요청
5. Poller는 select()를 호출하고 잠들어 있다가 차례대로 epoll_wait() -> select() 반환에 의해 깨어남
6. 워커 스레드 할당
7. 워커 스레드 일 처리
    - HTTP 요청 파싱
    - WebSocket 프로토콜로 전환
    - 응답 메시지 생성
8. 응답 메시지 write()
    - 우선적으로 JVM Direct Buffer에 쓰기
    - 이후 커널 계층의 송신 버퍼에 씀
9. WebSocket 프로토콜 전환 및 연결 성립
...
### ✨ 가설 7: WS Connection 후 워커 스레드를 할당 받는 과정에서 지연이 일어났을 것이다. (거짓)
- 이렇게 된다면 가설 5 (스레드가 부족하여 큐에 요청이 쌓이고 있을 것이다.)가 참이어야 하는데, 거짓이므로 해당 가설도 거짓임을 알 수 있다. 

<br/>

### ✨ 가설 8: ThreadPoolTaskExecutor의 풀 사이즈를 적게 잡아 대기 시간이 길어지며 튀는 현상이 발생했다. (거짓)

ThreadPoolTaskExecutor은 웹소켓 연결 후 코드 실행 로직 수행 시 사용되는 커스텀 비동기 풀이다.
- 알고 보니 내가 비동기 처리용 풀을 커스텀하여 CorePool 사이즈를 5개, MaxPool 사이즈를 10개로 설정해 둔 것이었다.
- 따라서 해당 부분에서 지연이 발생할 수도 있다는 생각을 하였다.
...
- 무엇보다 테스트 결과, `Send Code`와 `Check Result` 사이의 p99값이 0.1초이므로 해당 가설도 거짓이다. (WS 연결 후 요청을 보내고 응답을 받기까지의 시간)

<br/>

### ✨ 가설 9: Poller에 등록된 소켓이 Poller loop 경계에 걸리며 첫 처리까지 1초를 기다린다. (거짓)
- 그러나 1초로 지연이 일정하며, 꼬리에서 일부 요청만 튀는 현상이 발생하였다.
- Poller가 select()를 호출하자마자 소켓 요청이 들어와 다음 Poller loop로 처리되는 시점이 밀릴 때 지연이 발생하는 건가 싶었지만, 가설 6과 마찬가지로 selectorTimeout의 값 변화에도 여전히 1,000ms의 지연 시간을 보였으므로 거짓이다.
...
- Recv-Q는 0인데, 지연 시간이 1초로 튀는 모습으로 보아, Poller 스레드가 과부하되어 I/O 처리에 지연이 일어나 문제가 발생한다고 생각하였다.
- 그래서 Poller가 많은 양의 소켓을 처리하지 못하고 병목이 발생하였다고 추측하였다.

<br/>

- 이를 검증하기 위해 `PollerThreadCount`의 값을 1 -> 2로 조정하여 테스트를 수행하였다.
- Multi-Poller 구조 전환을 통해 하나의 Poller가 처리하는 이벤트가 절반으로 줄어들 것이라고 생각했기 때문이다.
- 이를 통해 I/O 환경에서 Poller가 빠르게 동작함으로써 요청을 스레드와 연결해 줄 수 있다고 생각했다.
...
- `Max`: 1,220ms -> 223ms **<U>(81.7% 감소)</U>**
- `Connect WS(p99)`: 229ms -> 176ms **<U>(23.1% 감소)</U>**
- `Check result(Max)`: 366ms -> 230ms **<U>(37.2% 감소)</U>**
- `Close(Max)`: 207ms -> 296ms **<U>(43% 증가)</U>**
- `표준편차`: 50ms -> 29ms **<U>(42% 감소)</U>**

미미하지만 Close()를 제외한 전체 성능이 소폭 개선되었음을 알 수 있다.
...
또한, 표준편차가 42% 감소하였다는 사실을 알 수 있는데, 이는 시스템의 안정성이 조금이나마 확보된 결과로 볼 수 있을 것이다!

![alt text](../../assets/image/Performance/sleepTest/request.png)

초당 처리율도 눈에 띄게 좋아졌음을 확인할 수 있다.

연결 종료 시에는 특이하게도 증가하였는데, p95부터 증가하는 모습을 보인다. 따라서 Multi-Puller 도입에 의해 무언가 변화가 생겼음을 의심할 수 있을 것이다.
...
- [자바에서는 멀티플렉싱을 어떻게 지원할까? – Java NIO의 Selector](https://ego2-1.tistory.com/29)